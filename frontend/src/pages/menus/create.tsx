...
export const MenuCreate = () => {
  const { formProps, saveButtonProps } = useForm({ resource: "menus", redirect: "show" });
  ...
  const addIngredient = () => {
    ...
    if (ingredients.find((i) => i.ingredientId === selectedIngId)) return;
  ...
  const ingColumns = [
    {
      ...
      render: (_: any, r: any, idx: number) => (
        <Select
          size="small"
          options={COURS_OPTIONS}
          value={r.coursType}
          onChange={(v) => updateRow(idx, "coursType", v)}
          style={{ width: "100%" }}
          allowClear
          placeholder="—"
        />
    ...
    },
    {
      ...
      render: (_: any, r: any, idx: number) => (
        <InputNumber size="small" min={0} step={0.0001} value={r.prixUnitaire}
          ...
          placeholder={r._ing?.prixUnitaire?.toFixed(4)} />
      ),
    },
    {
  ...
  const ingOptions = allIngredients
    ...
    .map((i) => ({ value: i.id, label: `${i.nom}${i.epuise ? " (épuisé)" : ""}` }));
  ...
  return (
    <Create saveButtonProps={saveButtonProps}>
      <Form {...formProps} layout="vertical" onFinish={handleFinish}>
        <Row gutter={16}>
          ...
          <Col xs={24} md={8}>
            <Form.Item label="Type de repas" name="typeRepas">
              <Select options={TYPE_OPTIONS} allowClear placeholder="Choisir..." />
            </Form.Item>
          </Col>
          <Col xs={24} md={8}>
            <Form.Item label="Nom du menu" name="nom">
              <Input placeholder="Ex: Menu printemps" />
            </Form.Item>
          </Col>
        </Row>
        ...
        <Row gutter={16}>
          ...
          <Col xs={12} md={8}>
            <Form.Item label="Couverts réels" name="nbCouvertsReels">
              <InputNumber min={0} style={{ width: "100%" }} placeholder="Après le service" />
            </Form.Item>
          </Col>
          <Col xs={12} md={8}>
            <Form.Item label="Couverts bénévoles" name="nbCouvertsBenevoles">
              <InputNumber min={0} style={{ width: "100%" }} placeholder="Repas offerts" />
            </Form.Item>
          </Col>
        </Row>
        ...
        <Row gutter={16}>
          <Col xs={12} md={6}>
            <Form.Item label="Bénévoles" name="nbBenevoles" initialValue={0}>
              <InputNumber min={0} style={{ width: "100%" }} />
            </Form.Item>
          </Col>
          <Col xs={12} md={6}>
            <Form.Item label="Heures bénévoles (total)" name="heuresBenevoles" initialValue={0}>
              <InputNumber min={0} step={0.5} style={{ width: "100%" }} />
            </Form.Item>
          </Col>
          <Col xs={12} md={12}>
            <Form.Item label="Chiffre d'affaires (€)" name="chiffreAffaires">
              <InputNumber min={0} step={0.01} style={{ width: "100%" }} placeholder="Saisir après le service" />
            </Form.Item>
          </Col>
        </Row>
        ...
        <div style={{ marginBottom: 8 }}>
          <Text strong>Ingrédients utilisés</Text>
        ...
        <Space style={{ marginBottom: 12 }}>
          <Select showSearch style={{ width: 280 }} placeholder="Choisir un ingrédient..."
